...
const char *javahead = "HTTP/1.1 200 OK\nContent-Type: text/javascript\nConnection: close\nContent-Length: ";
...
optval = 1;
setsockopt(new_socket, SOL_SOCKET, TCP_CORK, (const void *)&optval , sizeof(int));
printf ("resource is: %d bytes\n", l1);
valread = sprintf(buffer, "%s%d\n\n", javahead, l1);
write (new_socket, buffer, valread);
d1 = 0;
l2 = sendfile(new_socket, n, 0, (size_t)l1);
close (n);
printf("l2: %d\n", l2);
