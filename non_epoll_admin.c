...
struct epoll_event *inevents;
    inevents = calloc(maxevents, sizeof(struct epoll_event));
...
clientevent.events = EPOLLIN | EPOLLHUP | EPOLLOUT | EPOLLERR | EPOLLHUP;
...
inbuff.len = read (connfd, inbuff.p, inbuff.max);
if (inbuff.len == -1)
{usleep (1000); logging ("reading blocked!",1,0); continue;}
}// while read
...
if(a < 0)
{
{usleep (1000); logging ("closing, remote, not ready",1,0);}
//exit(1);
}
if(!a) 
{
