...
char *upuri;
char *uphandle;
}settings = {0, 1000, 100000, "upload.cst", "[* UPLOAD FILE *]"};
...
{
getcwd (request.path.p, 200);
request.mode = root;
}else{

midstr (request.uri.p, request.path.p , 1, request.uri.len);
request.path.len = request.uri.len - 1;
...
stradd (outbuff.p, request.path.p, &outbuff.len);
...
stradd (b1.p, "<form enctype=\"multipart/form-data\" action=\"", &b1.len);
//if (request.mode == dir)
stradd (b1.p, settings.upuri, &b1.len);
stradd (b1.p, "\" method=\"post\">\n", &b1.len);

stradd (b1.p, "<input type=\"file\" name=\"myFile\">\n", &b1.len);

stradd (b1.p, "<input type=\"submit\" value=\"upload\">\n", &b1.len);
